...
<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
      ...
      <ul class="flex flex-col gap-0.5">
        {#each data.recentNotes as note}
          ...
          <li>
            <a
              {href}
              class="hover:bg-accent flex items-start justify-between rounded-md px-2 py-1.5 transition-colors"
            >
              <div class="min-w-0">
                <p class="truncate text-sm">{note.title}</p>
                <p class="text-muted-foreground truncate text-xs">{spaceName}</p>
              </div>
              <span class="text-muted-foreground ml-3 shrink-0 text-xs tabular-nums">
  ...
  <section>
    <div class="mb-3 flex items-center gap-2">
      <CheckSquare class="text-muted-foreground size-4" />
      <h2 class="text-sm font-semibold">Open Todos</h2>
    </div>

    {#if data.openTodos.length === 0}
      <p class="text-muted-foreground text-sm italic">Nothing open — great work!</p>
    {:else}
      <ul class="flex flex-col gap-0.5">
          ...
          <li>
            ...
            >
              ...
              {#if badge}
                ...
                >
                  {new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric" }).format(new Date(todo.dueDate!))}
                </span>
              {/if}
            </a>
          </li>
