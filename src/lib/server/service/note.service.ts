...
export function getNotesBySpace(spaceId: string) {
	return db.select().from(notes).where(eq(notes.spaceId, spaceId)).orderBy(asc(notes.title)).all();
}
...
export function getNotesByRootSpace(rootSpace: string) {
	return db
		.select()
		.from(notes)
		.where(or(eq(notes.spaceId, rootSpace), sql`${notes.spaceId} LIKE ${rootSpace + '/%'}`))
