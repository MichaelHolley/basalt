<script lang="ts">
	...
	let filteredNotes = $derived(currentType === 'note' ? allNotes.filter((n) => n.id !== currentId) : allNotes);
	let filteredTodos = $derived(currentType === 'todo' ? allTodos.filter((t) => t.id !== currentId) : allTodos);
</script>
...
<div class="flex flex-col gap-1">
	<div class="mb-2 flex items-center justify-between">
		...
		<Button
			...
			size="icon"
			class="size-5"
			onclick={() => { addingRelation = !addingRelation; relationTargetId = ''; }}
			title="Add relation"
		>
			<Plus class="size-3" />
		</Button>
